            ...
            <div className="space-y-10">
              {education.map((edu, index) => (
                ...
                >
                  ...
                  >
                    <Card className="p-5 bg-white dark:bg-gray-800 shadow-md rounded-lg border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow duration-200">
                      <div className="mb-4">
                          ...
                          <div className="flex-shrink-0">
                            {edu.logo ? (
                              <img
                                src={edu.logo}
                                alt={`${edu.school} logo`}
                                className="w-12 h-12 sm:w-14 sm:h-14 object-contain rounded-lg bg-white dark:bg-gray-100 p-1 border border-gray-200 dark:border-gray-300 shadow-sm relative sm:mt-0 mt-1"
                                onError={(e) => {
                            ...
                            {/* Fallback Icon */}
                            <div 
                              className={`w-12 h-12 sm:w-14 sm:h-14 bg-blue-100 dark:bg-blue-900 rounded-lg flex items-center justify-center border border-gray-200 dark:border-gray-700 ${edu.logo ? 'hidden' : 'flex'}`}
                              style={{ display: edu.logo ? 'none' : 'flex' }}
                            >
                              <GraduationCap className="w-6 h-6 sm:w-7 sm:h-7 text-blue-600 dark:text-blue-400" />
                            </div>
                          </div>

                          {/* Degree and School Info */}
                          <div className="flex-1 min-w-0">
                            <h4 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white sm:mb-1 break-words">
                              {edu.degree}
                            </h4>
                            <div className="text-sm sm:text-md font-medium text-gray-700 dark:text-gray-300 mb-2 break-words">
                              <span>{edu.school}</span>
                            </div>
                          </div>
                        </div>

                        <div className="flex flex-col sm:flex-row sm:items-center text-sm text-gray-500 dark:text-gray-400 mb-2 gap-1 sm:gap-3">
                          <div className="flex items-center">
                      ...
                      <div className="mb-5">
                        <h5 className="font-semibold text-gray-900 dark:text-white mb-2 flex items-center text-sm">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {edu.coursework.map((course, courseIndex) => (
                            <Badge
                              key={courseIndex}
                              variant="secondary"
                              className="text-xs bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors cursor-pointer break-words"
                            >
                              {course}
                            </Badge>
                          ))}
                          ...
                          <ul className="space-y-1.5">
                            {edu.projects.map((project, projIndex) => (
                              ...
                              >
                                ...
                                <span className="group-hover:text-gray-800 dark:group-hover:text-gray-200 transition-colors break-words">
                                  {project}
                                </span>
                              </li>
                            ))}
